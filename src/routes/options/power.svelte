<script>
    ...
    import { name, user, program, workouts, exerciseName, muscleGroup, numSets, numReps, weightPercentage } from "$lib/stores";
    ...
    async function addWorkout(){
        if($exerciseName.length == 0 || $muscleGroup.length == 0 || $numSets.length == 0 || $numReps.length == 0 || $weightPercentage.length == 0) {
            ...
            const { error } = await supabase
            .from('workoutoptions')
            .insert([{program: 'power', exerciseName: $exerciseName, muscleGroup: $muscleGroup, numSets: $numSets, numReps: $numReps, weightPercentage: $weightPercentage}])
