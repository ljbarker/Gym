<script>
    ...
    async function addWorkout(){
        if($exerciseName.length == 0 || $muscleGroup.length == 0 || $numSets.length == 0 || $numReps.length == 0 || $weightPercentage.length == 0) {
            ...
            const { error } = await supabase
            .from('workoutoptions')
            .insert([{program: 'weight', exerciseName: $exerciseName, muscleGroup: $muscleGroup, numSets: $numSets, numReps: $numReps, weightPercentage: $weightPercentage}])
        ...
        <div class="px-2">
            <div class="relative flex items-center justify-between h-16">
                ...
                <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                    ...
                    <div>
                        <button type="button" on:click={() => {menuvis=!menuvis}} class="bg-gray-800 flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
